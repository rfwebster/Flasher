...
class Window(tk.Tk):
    ...
    def setup_ui(self):
        ...
        self.vac_frame = tk.LabelFrame(self.mainframe, text="Vacuum Values:")
        self.vac_frame.pack(fill=tk.X)

        self.SIP1Vac_label = ttk.Label(self.vac_frame, text="1st Int Chamber (SIP1):", width=20)
        self.SIP1Vac_label.grid(row=1, column=0, padx=PAD_X, pady=PAD_Y, sticky=tk.W)
        self.SIP1var = tk.StringVar(value="2.2")
        self.SIP1_spin = ttk.Spinbox(self.vac_frame, from_=0, to=10, increment=0.1, width=4, font=label_font, textvariable=self.SIP1var)
        self.SIP1_spin.grid(row=1, column=1, sticky=tk.E)
        self.SIP1exp_label = ttk.Label(self.vac_frame, text="x10^", width=4)
        self.SIP1exp_label.grid(row=1, column=2, sticky=tk.E)
        self.SIP1expvar = tk.StringVar(value="-8")
        self.SIP1exp_spin = ttk.Spinbox(self.vac_frame, from_=-9, to=-3, increment=1, width=2, font=label_font, textvariable=self.SIP1expvar)
        self.SIP1exp_spin.grid(row=1, column=3, sticky=tk.E)
        self.SIP1unit_label = ttk.Label(self.vac_frame, text="Pa", width=4)
        self.SIP1unit_label.grid(row=1, column=4, sticky=tk.E)

        self.SIP2Vac_label = ttk.Label(self.vac_frame, text="2nd Int Chamber (SIP 2):", width=20)
        self.SIP2Vac_label.grid(row=2, column=0, padx=PAD_X, pady=PAD_Y, sticky=tk.W)
        self.SIP2var = tk.StringVar(value="7.8")
        self.SIP2_spin = ttk.Spinbox(self.vac_frame, from_=0, to=10, increment=0.1, width=4, font=label_font, textvariable=self.SIP2var)
        self.SIP2_spin.grid(row=2, column=1,  sticky=tk.E)
        self.SIP2exp_label = ttk.Label(self.vac_frame, text="x10^", width=4)
        self.SIP2exp_label.grid(row=2, column=2, sticky=tk.E)
        self.SIP2expvar = tk.StringVar(value="-6")
        self.SIP2exp_spin = ttk.Spinbox(self.vac_frame, from_=-9, to=-3, increment=1, width=2, font=label_font, textvariable=self.SIP2expvar)
        self.SIP2exp_spin.grid(row=2, column=3,  sticky=tk.E)
        self.SIP2unit_label = ttk.Label(self.vac_frame, text="Pa", width=4)
        self.SIP2unit_label.grid(row=2, column=4, sticky=tk.E)

        self.SIP200Vac_label = ttk.Label(self.vac_frame, text="Accel Tube (SIP 200L):", width=20)
        self.SIP200Vac_label.grid(row=3, column=0, padx=PAD_X, pady=PAD_Y, sticky=tk.W)
        self.SIP200var = tk.StringVar(value="1.3")
        self.SIP200SIP_spin = ttk.Spinbox(self.vac_frame, from_=0, to=10, increment=0.1, width=4, font=label_font, textvariable=self.SIP200var)
        self.SIP200SIP_spin.grid(row=3, column=1,  sticky=tk.E)
        self.SIP200exp_label = ttk.Label(self.vac_frame, text="x10^", width=4)
        self.SIP200exp_label.grid(row=3, column=2, sticky=tk.E)
        self.SIP200expvar = tk.StringVar(value="-7")
        self.SIP200exp_spin = ttk.Spinbox(self.vac_frame, from_=-9, to=-3, increment=1, width=2, font=label_font, textvariable=self.SIP200expvar)
        self.SIP200exp_spin.grid(row=3, column=3, sticky=tk.E)
        self.SIP200unit_label = ttk.Label(self.vac_frame, text="Pa", width=4)
        self.SIP200unit_label.grid(row=3, column=4, sticky=tk.E)

        self.SIP3_label = ttk.Label(self.vac_frame, text="Column (SIP 3):", width=20)
        self.SIP3_label.grid(row=4, column=0, padx=PAD_X, pady=PAD_Y, sticky=tk.W)
        ...
        self.SIP3_spin.grid(row=4, column=1, sticky=tk.E)
        self.SIP3exp_label = ttk.Label(self.vac_frame, text="x10^", width=4)
        self.SIP3exp_label.grid(row=4, column=2, sticky=tk.E)
        ...
        self.SIP3exp_spin.grid(row=4, column=3,sticky=tk.E)
        self.SIP3unit_label = ttk.Label(self.vac_frame, text="Pa", width=4)
        self.SIP3unit_label.grid(row=4, column=4, sticky=tk.E)
    ...
    def get_vac_values(self):
        self.SIP1var = self.SIP1_spin.get()
        self.SIP1expvar = self.SIP1exp_spin.get()
        self.SIP2var = self.SIP2_spin.get()
        self.SIP2expvar = self.SIP2exp_spin.get()
        self.SIP200var = self.SIP200SIP_spin.get()
        self.SIP200expvar = self.SIP200exp_spin.get()
        self.SIP3var = self.SIP3_spin.get()
        self.SIP3expvar = self.SIP3exp_spin.get()

        self.SIP1 = self.SIP1var + "E" + self.SIP1expvar
        self.SIP2 = self.SIP2var + "E" + self.SIP2expvar
        self.SIP200 = self.SIP200var + "E" + self.SIP200expvar
        self.SIP3 = self.SIP3var + "E" + self.SIP3expvar

    def get_last_vac_values(self):
        last_entry = self.read_last_line(LOG_FILE)
        if last_entry:
            self.SIP1 = last_entry[4]
            self.SIP2 = last_entry[5]
            self.SIP200 = last_entry[6]
    ...
    def set_vac_values(self):
        ...
        try:
            self.SIP1_spin.delete(0,tk.END)
            self.SIP1_spin.insert(tk.END, self.SIP1.split("E")[0])
            self.SIP1exp_spin.delete(0,tk.END)
            self.SIP1exp_spin.insert(tk.END, self.SIP1.split("E")[1])
        except IndexError: 
            self.SIP1_spin.delete(0,tk.END)
            self.SIP1_spin.insert(tk.END, "0")
            self.SIP1exp_spin.delete(0,tk.END)
            self.SIP1exp_spin.insert(tk.END, "-9")
        try:
            self.SIP2_spin.delete(0,tk.END)
            self.SIP2_spin.insert(tk.END, self.SIP2.split("E")[0])
            self.SIP2exp_spin.delete(0,tk.END)
            self.SIP2exp_spin.insert(tk.END, self.SIP2.split("E")[1])
        except IndexError:
            self.SIP2_spin.delete(0,tk.END)
            self.SIP2_spin.insert(tk.END, "0")
            self.SIP2exp_spin.delete(0,tk.END)
            self.SIP2exp_spin.insert(tk.END, "-9")
        try:
            self.SIP200SIP_spin.delete(0,tk.END)
            self.SIP200SIP_spin.insert(tk.END, self.SIP200.split("E")[0])
            self.SIP200exp_spin.delete(0,tk.END)
            self.SIP200exp_spin.insert(tk.END, self.SIP200.split("E")[1])
        except IndexError:
            self.SIP200SIP_spin.delete(0,tk.END)
            self.SIP200SIP_spin.insert(tk.END, "0")
            self.SIP200exp_spin.delete(0,tk.END)
            self.SIP200exp_spin.insert(tk.END, "-9")
        try:
            ...
            self.SIP3_spin.delete(0,tk.END)
            self.SIP3_spin.insert(tk.END, "0")
            self.SIP3exp_spin.delete(0,tk.END)
            self.SIP3exp_spin.insert(tk.END, "-9")
    ...
    def logit(self):
        ...
        self.add_line_to_csv(LOG_FILE, [
            ...
            str(self.SIP1), str(self.SIP2), str(self.SIP200), str(self.SIP3),
            self.ht, self.EC,
            self.A1, self.A2,
            self.flashtype,
            self.notes]
